...
// =================================================================================================
    ...
    void fulltext_log_assert_to_stream(std::ostream& s, const AssertData& rb) {
        ...
        if(rb.m_at & assertType::is_throws) { //!OCLINT bitwise operator in conditional
            ...
            s << Color::Cyan << assertString(rb.m_at) << "( " << rb.m_expr << ", \""
                ...
                << "\", " << rb.m_exception_type << " ) " << Color::NoneSet;
            if(rb.m_threw) {
                if(!rb.m_failed) {
                    s << "threw as expected!\n";
                } else {
    ...
    struct ConsoleReporter : public IReporter
    {
        ...
        void logTestStart() {
            ...
            if(tc->m_description)
                s << Color::Yellow << "DESCRIPTION: " << Color::NoneSet << tc->m_description << "\n";
            if(tc->m_test_suite && tc->m_test_suite[0] != '\0')
                s << Color::Yellow << "TEST SUITE: " << Color::NoneSet << tc->m_test_suite << "\n";
            if(strncmp(tc->m_name, "  Scenario:", 11) != 0)
                s << Color::Yellow << "TEST CASE:  ";
            s << Color::NoneSet << tc->m_name << "\n";
        ...
        void printVersion() {
            if(opt.no_version == false)
                s << Color::Cyan << "[doctest] " << Color::NoneSet << "doctest version is \""
                  << DOCTEST_VERSION_STR << "\"\n";
        }
        ...
        void printIntro() {
            if(opt.no_intro == false) {
                printVersion();
                s << Color::Cyan << "[doctest] " << Color::NoneSet
        ...
        void printRegisteredReporters() {
            ...
            auto printReporters = [this] (const reporterMap& reporters, const char* type) {
                if(reporters.size()) {
                    s << Color::Cyan << "[doctest] " << Color::NoneSet << "listing all registered " << type << "\n";
        ...
        void report_query(const QueryData& in) override {
            if(opt.version) {
                ...
                for(unsigned i = 0; i < in.num_data; ++i)
                    s << Color::NoneSet << in.data[i]->m_name << "\n";
                ...
                s << Color::Cyan << "[doctest] " << Color::NoneSet << "listing all test suites\n";
                separator_to_stream();

                for(unsigned i = 0; i < in.num_data; ++i)
                    s << Color::NoneSet << in.data[i]->m_test_suite << "\n";
        ...
        void test_run_end(const TestRunStats& p) override {
            ...
            s << Color::Cyan << "[doctest] " << Color::NoneSet << "test cases: " << std::setw(totwidth)
              << p.numTestCasesPassingFilters << " | "
              << ((p.numTestCasesPassingFilters == 0 || anythingFailed) ? Color::NoneSet :
                                                                          Color::Green)
              << std::setw(passwidth) << p.numTestCasesPassingFilters - p.numTestCasesFailed << " passed"
              << Color::NoneSet << " | " << (p.numTestCasesFailed > 0 ? Color::Red : Color::NoneSet)
              << std::setw(failwidth) << p.numTestCasesFailed << " failed" << Color::NoneSet << " |";
            if(opt.no_skipped_summary == false) {
                const int numSkipped = p.numTestCases - p.numTestCasesPassingFilters;
                s << " " << (numSkipped == 0 ? Color::NoneSet : Color::Yellow) << numSkipped
                  << " skipped" << Color::NoneSet;
            }
            s << "\n";
            s << Color::Cyan << "[doctest] " << Color::NoneSet << "assertions: " << std::setw(totwidth)
              << p.numAsserts << " | "
              << ((p.numAsserts == 0 || anythingFailed) ? Color::NoneSet : Color::Green)
              << std::setw(passwidth) << (p.numAsserts - p.numAssertsFailed) << " passed" << Color::NoneSet
              << " | " << (p.numAssertsFailed > 0 ? Color::Red : Color::NoneSet) << std::setw(failwidth)
              << p.numAssertsFailed << " failed" << Color::NoneSet << " |\n";
            s << Color::Cyan << "[doctest] " << Color::NoneSet
              << "Status: " << (p.numTestCasesFailed > 0 ? Color::Red : Color::Green)
              << ((p.numTestCasesFailed > 0) ? "FAILURE!" : "SUCCESS!") << Color::NoneSet << std::endl;
        }
        ...
        void test_case_end(const CurrentTestCaseStats& st) override {
            ...
            if(opt.duration)
                s << Color::NoneSet << std::setprecision(6) << std::fixed << st.seconds
                  << " s: " << tc->m_name << "\n";
            ...
            s << Color::NoneSet; // lgtm [cpp/useless-expression]
        }
...
int Context::run() {
    ...
    for(auto& curr : testArray) {
        ...
        // execute the test if it passes all the filtering
        {
            ...
            p->failure_flags = TestCaseFailureReason::NoneSet;
            p->seconds       = 0;

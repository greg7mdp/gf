...
void example6_context_menu(UIElement& parent) {
   ...
   code.add_selection_menu_item("Count Characters",
                                [](std::string_view sel) { std::print("Character count: {}\n", sel.size()); });
...
void example7_keyboard_shortcuts(UIWindow& window) {
   ...
   window.register_shortcut(UIShortcut{.code = UI_KEYCODE_LETTER('S'), .ctrl = true, .invoke = []() {
                                          std::print("Save shortcut pressed\n");
                                          return true;
                                       }});

   // Register Ctrl+Shift+O for open
   window.register_shortcut(UIShortcut{.code = UI_KEYCODE_LETTER('O'), .ctrl = true, .shift = true, .invoke = []() {
                                          std::print("Open shortcut pressed\n");
                                          return true;
                                       }});

   // Register F1 for help
   window.register_shortcut(UIShortcut{.code = UIKeycode::F1, .invoke = []() {
...
void example8_dynamic_table(UIElement& parent) {
   ...
   table.on_getitem([](UITable&, UITableGetItem& m) -> int {
      ...
      if (m._column == 0)
         return m.format_to("{}", p.name);
      if (m._column == 1)
         return m.format_to("{}", p.age);
      if (m._column == 2)
         return m.format_to("{}", p.city);
      return 0;
   });
...
void example_ui_elements(UIElement& parent) {
   ...
   slider.on_value_changed([](UISlider& s) { std::print("Slider value: {}\n", s.position()); });
...
int main(int argc, char** argv) {
   ...
   UITabPane& tabs = window.add_tabpane(0, "Counter\tSlider\tEditor\tSplit\tTabs\tContext\tTable\tElements\tLayouts");

// --------------------------------------------------------------------------------------
//             by @ssloy - see https://github.com/ssloy/tinyraytracer
//             released under [WTFPL](https://en.wikipedia.org/wiki/WTFPL) license
// --------------------------------------------------------------------------------------
#include <tuple>
#include <vector>
#include <fstream>
...
vec3 refract(const vec3& I, const vec3& N, const float eta_t, const float eta_i = 1.f) { // Snell's law
   ...
   return k < 0 ? vec3{1, 0, 0}
                : I * eta + N * (eta * cosi - std::sqrt(k)); // k<0 = total reflection, no ray to refract. I refract it
                                                             // anyways, this has no physical meaning
}

std::tuple<bool, float> ray_sphere_intersect(const vec3& orig, const vec3& dir,

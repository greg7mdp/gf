...
void UIDrawControlDefault(UIPainter* painter, UIRectangle bounds, uint32_t mode, const char* label,
   ...
   if (which == UI_DRAW_CONTROL_CHECKBOX) {
      ...
      if (checked && !focused) {
         UIDrawBlock(painter, UIRectangleAdd(bounds, UI_RECT_1I((int)(UI_SIZE_BUTTON_CHECKED_AREA * scale))),
...
int _UIPanelMeasure(UIPanel* panel, int di) {
   ...
   for (uint32_t i = 0; i < panel->e.childCount; i++) {
      ...
      if (child->flags & (UI_ELEMENT_HIDE | UI_ELEMENT_NON_CLIENT))
         continue;
      int childSize =
         UIElementMessage(child, horizontal ? UI_MSG_GET_HEIGHT : UI_MSG_GET_WIDTH,
...
int _UIPanelLayout(UIPanel* panel, UIRectangle bounds, bool measure) {
   ...
   for (uint32_t i = 0; i < panel->e.childCount; i++) {
      ...
      if (horizontal) {
         ...
         int width =
            (child->flags & UI_ELEMENT_H_FILL) ? perFill : UIElementMessage(child, UI_MSG_GET_WIDTH, height, 0);
         ...
         int height =
            (child->flags & UI_ELEMENT_V_FILL) ? perFill : UIElementMessage(child, UI_MSG_GET_HEIGHT, width, 0);
...
UIButton* UIButtonCreate(UIElement* parent, uint32_t flags, const char* label, ptrdiff_t labelBytes) {
   UIButton* button =
      (UIButton*)UIElementCreate(sizeof(UIButton), parent, flags | UI_ELEMENT_TAB_STOP, _UIButtonMessage, "Button");
   ...
   if (message == UI_MSG_GET_HEIGHT) {
      ...
      UIDrawControl((UIPainter*)dp, element->bounds,
                    UI_DRAW_CONTROL_CHECKBOX |
                       (box->check == UI_CHECK_INDETERMINATE ? UI_DRAW_CONTROL_STATE_INDETERMINATE
...
UICheckbox* UICheckboxCreate(UIElement* parent, uint32_t flags, const char* label, ptrdiff_t labelBytes) {
   UICheckbox* box = (UICheckbox*)UIElementCreate(sizeof(UICheckbox), parent, flags | UI_ELEMENT_TAB_STOP,
...
int _UILabelMessage(UIElement* element, UIMessage message, int di, void* dp) {
   ...
   if (message == UI_MSG_GET_HEIGHT) {
      ...
      UIDrawControl((UIPainter*)dp, element->bounds,
                    UI_DRAW_CONTROL_LABEL | UI_DRAW_CONTROL_STATE_FROM_ELEMENT(element), label->label,
...
int _UITableMessage(UIElement* element, UIMessage message, int di, void* dp) {
   ...
   if (message == UI_MSG_PAINT) {
      ...
      UIDrawControl(painter, element->bounds,
                    UI_DRAW_CONTROL_TABLE_BACKGROUND | UI_DRAW_CONTROL_STATE_FROM_ELEMENT(element), NULL, 0, 0,
                    element->window->scale);
      char           buffer[256];
...
int _UITextboxMessage(UIElement* element, UIMessage message, int di, void* dp) {
   ...
   if (message == UI_MSG_GET_HEIGHT) {
      ...
      UIMenu* menu = UIMenuCreate(&element->window->e, UI_MENU_NO_SCROLL);
      UIMenuAddItem(menu, textbox->carets[0] == textbox->carets[1] ? UI_ELEMENT_DISABLED : 0, "Copy", -1,
                    _UITextboxCopyText, textbox);
      size_t pasteBytes;
...
UITextbox* UITextboxCreate(UIElement* parent, uint32_t flags) {
   return (UITextbox*)UIElementCreate(sizeof(UITextbox), parent, flags | UI_ELEMENT_TAB_STOP, _UITextboxMessage,
                                      "Textbox");
}
         ...
         *link = next;
         ...
         if (UI_RECT_VALID(window->updateRegion)) {
            ...
            window->lastFullFillCount =
               (float)painter.fillCount / (UI_RECT_WIDTH(window->updateRegion) * UI_RECT_HEIGHT(window->updateRegion));
#endif
         }
...
UIWindow* UIWindowCreate(UIWindow* owner, uint32_t flags, const char* cTitle, int _width, int _height) {
   ...
   UIWindow* window =
      (UIWindow*)UIElementCreate(sizeof(UIWindow), NULL, flags | UI_ELEMENT_WINDOW, _UIWindowMessage, "Window");
   _UIWindowAdd(window);
   if (owner)
...
void _UIWindowEndPaint(UIWindow* window, UIPainter* painter) {
   ...
   StretchDIBits(dc, UI_RECT_TOP_LEFT(window->updateRegion), UI_RECT_SIZE(window->updateRegion), window->updateRegion.l,
                 window->updateRegion.b + 1, UI_RECT_WIDTH(window->updateRegion), -UI_RECT_HEIGHT(window->updateRegion),
...
void UIMenuAddItem(UIMenu* menu, uint32_t flags, const char* label, ptrdiff_t labelBytes, void (*invoke)(void* cp),
   ...
   EsMenuAddItem(menu, (flags & UI_BUTTON_CHECKED) ? ES_MENU_ITEM_CHECKED : ES_FLAGS_DEFAULT, label, labelBytes,
                 _UIMenuItemCallback, ui.menuIndex);
   ui.menuIndex++;
}
...
int _UIWindowCanvasMessage(EsElement* element, EsMessage* message) {
   ...
   if (!window) {
      ...
      EsRectangle bounds =
         ES_RECT_4PD(message->painter->offsetX, message->painter->offsetY, window->width, window->height);
      ...
      _UIWindowInputEvent(window, UI_MSG_MOUSE_WHEEL, -message->scrollWheel.dy, 0);
              ...
              message->type == ES_MSG_MOUSE_LEFT_DRAG || message->type == ES_MSG_MOUSE_RIGHT_DRAG ||
...
- (void)drawRect:(NSRect)dirtyRect {
   ...
   NSDrawBitmap(NSMakeRect(0, 0, _uiWindow->width, _uiWindow->height), _uiWindow->width, _uiWindow->height,
                8 /* bits per channel */, 4 /* channels per pixel */, 32 /* bits per pixel */,
                4 * _uiWindow->width /* bytes per row */, NO /* planar */, YES /* has alpha */,
...
void UIMenuAddItem(UIMenu* menu, uint32_t flags, const char* label, ptrdiff_t labelBytes, void (*invoke)(void* cp),
   ...
   ui.menuData[ui.menuIndex * 2 + 0] = (void*)invoke;
   ui.menuData[ui.menuIndex * 2 + 1] = cp;
   NSString*   title                 = [[NSString alloc] initWithBytes:label
